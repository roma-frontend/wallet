...
import { buildParsePrompt, type ParsedTransaction } from "@/lib/ai-advisor";
...
export async function POST(req: NextRequest) {
  ...
  const ip =
    ...
    "unknown";
  const { allowed, reset } = await checkRateLimitAsync(`ai-parse:${ip}`);
  if (!allowed) {
  ...
  try {
    ...
    const { text: out } = await generateText({
      ...
      prompt: buildParsePrompt(text.slice(0, 300), {
    ...
    try {
      ...
      const raw = JSON.parse(json) as Partial<ParsedTransaction>;
      parsed = {
